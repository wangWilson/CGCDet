...
def single_gpu_test(model, data_loader, show=False, log_dir=None):
    ...
    for i, data in enumerate(data_loader):
           ...
           scale_factor=dataset.img_infos[i]['filename'].split('__')[1]

           with torch.no_grad():
               result = model(return_loss=False, rescale=not show,scale_factor=scale_factor, **data)
           results.append(result)

           if show:
               model.module.show_result(data, result,i, dataset.img_norm_cfg)

...
from mmdet.core import delta2dbbox, multiclass_nms_rbbox, \
    bbox_target_rbbox, accuracy, rbbox_target_rbbox,bbox_target,\
...
class BBoxHeadRbbox(nn.Module):
    ...
    # def get_target_bbox(self, sampling_results, gt_masks, gt_labels,
    def get_target_bbox(self, sampling_results, gt_bboxes, gt_labels,
                   rcnn_train_cfg):
        pos_proposals = [res.pos_bboxes for res in sampling_results]
        neg_proposals = [res.neg_bboxes for res in sampling_results]
        pos_gt_bboxes = [res.pos_gt_bboxes for res in sampling_results]
    ...
    def regress_by_class_rbbox(self, rois, label, bbox_pred, img_meta):
        ...
        if rois.size(1) == 5:##True
            if self.with_module:#False

...
def mask_cross_entropy(pred, target,label=None):
...
def ciou_loss(pred, target, beta=0.2, eps=1e-3, reduction='mean'):
...
def weighted_iou_loss(pred,
                      ...
                      eps=1e-3,
                      avg_factor=None):
    if style not in ['bounded', 'naive', 'ciou']:
    ...
    if style == 'bounded':
        ...
        loss = ciou_loss(pred[inds], target[inds], reduction='sum')
...
def _expand_binary_labels1(labels, label_weights, label_channels):

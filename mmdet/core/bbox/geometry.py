...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False):
    ...
    if is_aligned:
        ...
        # if bboxes1.shape[0]>200 and bboxes2.shape[0]>300:
        #     if bboxes2.shape[0]>bboxes1.shape[0]:
        #        bboxes1=bboxes1[:200]
        #     else:
        #        bboxes2=bboxes2[:200]
        # print(bboxes1.shape,bboxes2.shape)
        if False:#min(bboxes1.shape[0],bboxes2.shape[0])>300:
                # print('cpu')
                box_device = bboxes1.device
                bboxes1_cpu = bboxes1.cpu().numpy().astype(np.float)
                bboxes2_cpu = bboxes2.cpu().numpy().astype(np.float)

                lt = np.maximum(bboxes1_cpu[:,None, :2], bboxes2_cpu[:, :2])
                rb = np.minimum(bboxes1_cpu[:,None, 2:], bboxes2_cpu[:, 2:])
                wh = np.clip(rb - lt + 1, a_min=0, a_max=None)
                overlap = wh[:, :, 0] * wh[:, :, 1]
                
                overlap = torch.from_numpy(overlap).to(box_device)
                # print('end1')
        else:
            # print('gpu')
            wh = (torch.min(bboxes1[:, None, 2:], bboxes2[:, 2:])-torch.max(bboxes1[:, None, :2], bboxes2[:, :2])+1).clamp(min=0) 
            overlap = wh[:, :, 0] * wh[:, :, 1]
        # if hasattr(torch.cuda,'empty_cache'):
        #     torch.cuda.empty_cache()
...
def rbbox_overlaps_cy_warp(rbboxes, query_boxes,method=0):
    ...
    for index in range(len(inds[0])):
        ...
        if method==0:
            ...
            overlap = polyiou.iou_poly_self_r(polyiou.VectorDouble(box), polyiou.VectorDouble(query_box))

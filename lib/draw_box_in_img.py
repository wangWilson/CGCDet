...
from .tools import get_dota_short_names
from .coordinate_convert import forward_convert
...
STANDARD_COLORS = [
    'AliceBlue', 'Chartreuse', 'Aqua', 'Aquamarine', 'Azure', 'Red', 'Bisque',
...
def draw_label_with_scores_csl(draw_obj, box, label, score, method, head, color='White'):
    ...
    if cfgs.DATASET_NAME.startswith('DOTA'):
        # print(123456798)
        label_name = get_dota_short_names(LABEL_NAME_MAP[label+1])
    else:
...
def draw_boxes_with_label_and_scores(img_array, boxes, labels, scores, method, head=None, is_csl=False, in_graph=True):
    ...
    img_array = np.array(img_array, dtype=np.uint8)#* 255 / np.max(img_array), dtype=np.uint8)
...
if __name__ == '__main__':
    img_array = cv2.imread("/home/PycharmProjects/FPN_TF/tools/inference_image/2.jpg")
